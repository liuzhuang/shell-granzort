...
import { dashboardMockTab } from '../lib/dashboard-mock'
...
const seedMessages: DashboardChatMessage[] = [
  { id: 'seed-1', role: 'ai' as const, text: '欢迎使用看板助手。请先选择命令，然后输入自然语言创建或修改看板。' },
...
function dataFromProbe(spec: DashboardWidgetSpec, response: DashboardExecuteProbeResponse): DashboardWidgetData {
  ...
  if (spec.kind === 'timeseries') {
    const points = Array.from({ length: 12 }).map((_, index) => ({
      label: `${index + 1}`,
      value: Number((Math.max(3, (stdout.length % 30) + Math.sin((Date.now() / 1000 + index) / 2) * 6 + 20)).toFixed(1))
    }))
    return { kind: 'timeseries', unit: 'MB/s', points }
  }
  const lines = stdout
    .split(/\r?\n/)
    .map((line) => line.trim())
...
export function DashboardPage() {
  ...
  const [savedDashboardTab, setSavedDashboardTab] = useState<DashboardTab>(dashboardMockTab)
  const [draftDashboardTab, setDraftDashboardTab] = useState<DashboardTab>(dashboardMockTab)
  ...
  const [approvedTokenMap, setApprovedTokenMap] = useState<Record<string, string>>({})
  ...
  const runRefresh = useCallback(async () => {
    ...
    try {
      ...
      const tasks = draftDashboardTab.widgets.map(async (widget) => {
        const step = widget.probe.steps[0]
        if (!step) return
        const key = `${widget.id}:${step.stepId}`
        const isSsh = isSshProbeCommand(step.command)
        const effectiveTimeoutMs = isSsh ? Math.max(45_000, Number(step.timeoutMs) || 0) : Math.max(5_000, Number(step.timeoutMs) || 0)
        const response = await window.api.dashboardExecuteProbe({
          ...
          tokenAuth: approvedTokenMap[key]
        })
        if (!response.success || response.isBlockedBySecurity) {
          if (widget.kind === 'metric') {
            ...
            nextDataMap[widget.id] = { kind: 'timeseries', unit: 'MB/s', points: [] }
          } else {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        const commands = (parsed?.commands || []).map((item) => ({ name: item.name, command: item.command }))
        setCommandOptions(commands)
        if (commands.length > 0) setSelectedCommandName(commands[0].name)
        if (cfg?.tabs?.length) {
    ...
    try {
      const result = await window.api.dashboardIntent({
        ...
        context: {
          ...
          baseConnectionCommand: selectedCommand?.command || `ssh ${draftDashboardTab.contextLabel}`,
  ...
  }, [draftDashboardTab, input, intentThreadId, lastGenerationFeedback, loadingIntent, messages, runRefresh, selectedCommand])
  ...
  const handleApprove = useCallback(async (widgetId: string, stepId: string, command: string) => {
    ...
    try {
      ...
      setApprovedTokenMap((prev) => ({ ...prev, [`${widgetId}:${stepId}`]: issued.tokenAuth }))
      await runRefresh()
    } finally {
