...
import { resolveAutoUpdateFeedBaseUrl } from './update-feed'
...
function registerDisabledAutoUpdateHandlers(reason: UpdateDisabledReason): void {
...
export function setupAutoUpdater(broadcast: (channel: string, payload: unknown) => void): void {
  ...
  const baseUrl = resolveAutoUpdateFeedBaseUrl()
  if (!baseUrl) {
    registerDisabledAutoUpdateHandlers('missing-feed-url')
    return
  }
