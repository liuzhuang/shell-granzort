...
const dashboardSubagents: SubAgent[] = [
  {
    ...
    systemPrompt: [
      ...
      '目标：提炼 targetDatasourceId、baseConnectionCommand、envInfo。',
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      '输出应覆盖 CPU、内存、磁盘、带宽、慢 SQL、事件流等可观测目标。',
...
function createModel(config: AppConfig): ChatOpenAI {
  ...
  const showRawKey = process.env.SHELL_MANAGE_DEBUG_SHOW_LLM_KEY === 'true'
  console.info('[dashboard][intent][deepagents] llm config', {
    provider,
    endpoint: maskEndpoint(endpoint),
    model: config.settings.llm.model,
    apiKey: showRawKey ? rawApiKey : maskApiKey(rawApiKey),
...
function applyLangSmithTracing(config: AppConfig): void {
...
function getCacheKey(config: AppConfig): string {
  return [config.settings.llm.provider || 'openai', config.settings.llm.endpoint || '', config.settings.llm.model || ''].join('::')
}
...
async function getAgent(config: AppConfig): Promise<DeepAgentLike> {
  ...
  const agent = createDeepAgent({
    ...
    systemPrompt: [
      ...
      '9) context.availableShellCommands 已提供候选连接命令，优先参考 context.selectedShellCommandName；probe.steps[].command 仅输出探针本体命令，不要自行再拼接 ssh 前缀。'
    ].join('\n')
  })
...
function applyLocalSemanticRepair(dashboard: DashboardTab): LocalSemanticRepairResult {
  ...
  for (const widget of repaired.widgets) {
    const widgetId = String(widget.id || '').trim() || 'widget'
    const steps = Array.isArray(widget.probe?.steps) ? widget.probe.steps : []
    if (steps.length === 0) {
      steps.push({
        stepId: `${widgetId}-step-1`,
        command: 'echo "no probe command generated"',
        shellType: 'bash',
        timeoutMs: 5000,
        riskLevel: 'safe'
      })
      fixes.push(`补全 probe step: widget(${widgetId}) 自动生成默认只读步骤`)
    }
    const used = new Set<string>()
    for (let index = 0; index < steps.length; index += 1) {
      const step = steps[index] as unknown as Record<string, unknown>
      const rawStepId = String(step.stepId || '').trim()
      const altId = String(step.id || '').trim()
      let candidate = rawStepId || altId || `${widgetId}-step-${index + 1}`
      if (!rawStepId) {
        fixes.push(`补全 stepId: widget(${widgetId}) step#${index + 1} -> ${candidate}`)
      }
      if (used.has(candidate)) {
        let suffix = 2
        while (used.has(`${candidate}-${suffix}`)) suffix += 1
        const deduped = `${candidate}-${suffix}`
        fixes.push(`修复重复 stepId: widget(${widgetId}) ${candidate} -> ${deduped}`)
        candidate = deduped
      }
      used.add(candidate)
      ;(steps[index] as { stepId: string }).stepId = candidate
      const rawCommand = String(step.command || '').trim()
      if (!rawCommand) {
        ;(steps[index] as { command: string }).command = 'echo "no probe command generated"'
        fixes.push(`补全 command: widget(${widgetId}) step(${candidate})`)
      }
      const rawRiskLevel = step.riskLevel
      if (!isRiskLevel(rawRiskLevel)) {
        ;(steps[index] as { riskLevel: 'safe' | 'review' | 'blocked' }).riskLevel = 'safe'
        fixes.push(`补全 riskLevel: widget(${widgetId}) step(${candidate}) -> safe`)
      }
      const rawShellType = step.shellType
      if (!isShellType(rawShellType)) {
        ;(steps[index] as { shellType: 'bash' | 'zsh' | 'mysql' | 'redis' }).shellType = 'bash'
        fixes.push(`补全 shellType: widget(${widgetId}) step(${candidate}) -> bash`)
      }
      const rawTimeoutMs = Number(step.timeoutMs)
      const isSsh = /^\s*ssh(\s|$)/i.test(String((steps[index] as { command?: string }).command || ''))
      if (!Number.isFinite(rawTimeoutMs) || rawTimeoutMs <= 0) {
        ;(steps[index] as { timeoutMs: number }).timeoutMs = isSsh ? 45000 : 5000
        fixes.push(`补全 timeoutMs: widget(${widgetId}) step(${candidate}) -> ${isSsh ? 45000 : 5000}`)
      } else if (isSsh && rawTimeoutMs < 45000) {
        ;(steps[index] as { timeoutMs: number }).timeoutMs = 45000
        fixes.push(`提升 timeoutMs: widget(${widgetId}) step(${candidate}) ${rawTimeoutMs} -> 45000`)
      }
    }
  }
