...
description: >-
  ...
  operational Q&A in Cursor or Claude Code.
compatibility: Cursor, Claude Code, macOS. Requires bash and ruby (preinstalled on macOS) for bundled scripts.
metadata:
  version: "1.0.0"
...
Bundled product knowledge is available directly in `references/` (works after `npx skills add` install).  
...
2. Produce one-line candidates: `cd <absolute_project_path> && <command>`
...
- **Command paths** — every `command` must include `cd <abs-path> &&`; relative paths break at runtime
